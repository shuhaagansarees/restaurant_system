...
# Delete the ES6 versions from the file
html = re.sub(r'function updateStatus\(orderId, newStatus\) \{.*?\try \{.*?\} catch\(e\) \{.*?\}\n    \}', '', html, flags=re.DOTALL)
html = re.sub(r'function updateCardButtons\(card, status\) \{.*?    \}', '', html, flags=re.DOTALL)
html = re.sub(r'function updateStatusAPI\(orderId, status\) \{.*?\}\n    \}', '', html, flags=re.DOTALL)

# Insert ES5 script block at the top of the content block
